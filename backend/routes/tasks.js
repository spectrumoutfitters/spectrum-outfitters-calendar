...
import {
  ...
  finalizeInventoryOnTaskCompleted,
    ...
    if (userIds.length > 0) {
      for (const userId of userIds) {
        ...
        sendPushToUser(userId, {
          ...
          body: `You have been assigned: ${toTitleCase(sanitizeInput(title))}`
        }).catch(() => {});
      }
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // Status→completed (TaskModal / employee mark-done) never hits /approve; finalize inventory once.
    if (status !== undefined) {
      await finalizeInventoryOnTaskCompleted(db, {
        taskId: id,
        userId: req.user.id,
...
router.put('/:id/status', async (req, res) => {
  try {
    ...
    // Kanban / status-only complete skips /approve; claim first finalize + deduct linked parts.
    await finalizeInventoryOnTaskCompleted(db, {
      taskId: id,
      userId: req.user.id,
      previousStatus: currentTask.status,
...
router.post('/:id/approve', requireAdmin, async (req, res) => {
  try {
    ...
    // Also covers tasks already completed via status PUT (inventory finalized there).
