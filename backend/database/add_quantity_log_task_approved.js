...
/**
 * Allow reason 'task_approved' in inventory_quantity_log (for auto-decrement when task is approved).
 * SQLite cannot alter CHECK, so we recreate the table.
 */
async function addQuantityLogTaskApproved() {
  try {
    await db.runAsync(`
      CREATE TABLE IF NOT EXISTS inventory_quantity_log_new (
        ...
        reason TEXT NOT NULL DEFAULT 'count' CHECK(reason IN ('count', 'refill_received', 'task_approved')),
    ...
    console.log('✅ inventory_quantity_log now allows reason task_approved');
  } catch (error) {
