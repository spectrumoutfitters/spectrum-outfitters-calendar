...
async function withFrontendModules(paths, fn) {
  const server = await createServer({ server: { middlewareMode: true }, appType: 'custom' });
  try {
    const modules = [];
    for (const path of paths) {
      modules.push(await server.ssrLoadModule(path));
    }
...
test('non-monthly pay stubs default to literal per-check gross amounts', async () => {
  await withFrontendModules(
    ['/src/components/Admin/PayStubMaker.jsx', '/src/utils/payStubPdf.js'],
    ({ DEFAULT_SPREAD_MONTHLY_ACROSS_PAYCHECKS }, { buildPreparedPaystubPages }) => {
...
test('monthly gross split remains available by explicit opt-in', async () => {
  await withFrontendModules(['/src/utils/payStubPdf.js'], ({ buildPreparedPaystubPages }) => {
    const [page] = buildPreparedPaystubPages(
      [{ periodEnd: '2026-06-05', gross: 1000 }],
      true,
      {},
